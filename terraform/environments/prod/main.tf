...
# Create PostgreSQL database with high availability for production
module "database" {
...
# Configure Cloud Armor WAF with more restrictive settings for production
module "cloud_armor" {
  ...
  rate_limiting_rules = {
    "api-endpoints" = {
      ...
      enforce_on_key  = "IP"
      threshold_count = 50  # More restrictive rate limiting
      interval_sec    = 60
    },
    "login-endpoints" = {
      expression      = "request.path.matches('/api/auth/login')"
      enforce_on_key  = "IP"
      threshold_count = 10  # More restrictive for login attempts
      interval_sec    = 60
    }
  }
...
module "backend_service" {
  ...
  cpu            = "2000m"
  memory         = "2048Mi"
  min_instances  = 2  # Higher min instances for reliability
  max_instances  = 20 # Higher max instances for scalability
...
module "frontend_service" {
  ...
  memory         = "1024Mi"
  min_instances  = 2  # Higher min instances for reliability
  max_instances  = 10 # Higher max instances for scalability
...
resource "google_compute_global_forwarding_rule" "https_redirect" {
  name       = "${var.project_prefix}-http-lb-rule"
